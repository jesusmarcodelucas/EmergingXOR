...
def run_phases(mode="full", w0=W0_BELOW_CLIFF, w_hl=W_HL_PER_SYN, eta=ETA_STEP,
               ...
               balanced=True, hub_shared_frac=1.0, w_hl_build=None):
    """w_hl_build is a CONTROL, not the model.  Leave it None for the thesis
    condition, in which one and the same hub->lateral conductance serves both
    phases and only ECl changes between them.  Setting it decouples the
    builder-phase weight from the governor-phase weight, which is useful only
    to ask which of the two roles produces which boundary of the window."""
    ...
    hl_build = hl if w_hl_build is None else (0 * nS if mode == "no_hub"
                                              else w_hl_build)
    S_hl = Synapses(Hub, Lat, model="w_sh : siemens", on_pre="gi_post += w_sh")
    S_hl.connect(True)
    S_hl.w_sh = hl_build
    ...
    if mode == "builder_only":
        net.run((t_ramp + t_gov) * second)          # no switch: keeps hl_build
